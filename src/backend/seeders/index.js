...
const populateIDs = (hasIDs, needsIDs, IDfieldOne, IDfieldTwo) => {
  if (needsIDs.length > hasIDs.length) {
    for (let i = 0; i < needsIDs.length; i++) {
      needsIDs[i][IDfieldOne] = hasIDs[Math.floor( Math.random() * 4 )].id;
      needsIDs[i][IDfieldTwo] = hasIDs[Math.floor( Math.random() * 4 )].id;
    }
  } else {
    for (let i = 0; i < needsIDs.length; i++) {
...
const runSeeds = async () => {
  ...
  await Product.bulkCreate(product, { raw: true });
